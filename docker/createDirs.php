...
if (count($argv) == 1) {
    ...
    $configFile = $argv[1];
    echo "A second argument has not been passed. The owner of the directories will be 'www-data'\n";
    $newOwner = "www-data";
} elseif (count($argv) == 3) {
    $configFile = $argv[1];
    $newOwner = $argv[2];
...
$outputCmd = [];
exec("id $newOwner",$outputCmd,$exitCode);
if ($exitCode !== 0) {
...
//Conf directory
exec("chmod -R 755 $aspenDir/sites/$siteName/conf");
exec("chown $newOwner $aspenDir/sites/$siteName/conf");
exec("chown $newOwner $aspenDir/sites/$siteName/conf/config*");
...
//Site directory
exec("chown root:root $aspenDir/sites/$siteName/httpd-$siteName.conf");
exec("chown root:root $aspenDir/sites/$siteName/conf/crontab_settings.txt");
exec("chmod 0644 $aspenDir/sites/$siteName/conf/crontab_settings.txt");

if (file_exists("$aspenDir/sites/$siteName/conf/log4j")) {
    exec("chown $newOwner /usr/local/aspen-discovery/sites/$siteName/conf/log4j*");
}
if (file_exists("$aspenDir/sites/$siteName/conf/passkey")) {
    exec("chown $newOwner $aspenDir/sites/$siteName/conf/passkey");
}
